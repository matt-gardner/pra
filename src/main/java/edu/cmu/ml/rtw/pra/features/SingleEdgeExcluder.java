...
public class SingleEdgeExcluder implements EdgeExcluder {
    ...
    public boolean shouldExcludeEdge(int sourceVertex,
                                     ...
                                     int edgeType) {
        // We can use edges of the relation we're predicting as long as they aren't the instance
        // we're starting the walk from.  So only exclude the walk if the source vertex is the same
        // as the current vertex or the next vertex
        ...
        // If we've made it here, we actually have a (source, target) pair that was given to us in
        // the edgesToExclude parameter.  Now we have to check if the edge we're trying to walk on
        // should be excluded.
        int edgeCount = 0;
        ...
        // But if it did show up, we still might be ok, if there are more edges between the source
        // and target node than just the ones we wanted to exclude.  This shouldn't happen with
        // standard KB edges, but if the edges are embedded into a latent space, there could be
        // other edges that have the same representation, so we have to check the counts here.
