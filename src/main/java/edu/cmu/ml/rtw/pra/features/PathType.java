...
public interface PathType {
    ...
    /**
     * How many iterations should we run if we want to be sure we complete the computation for
     * this path type?
     */
    ...
    /**
     * Creates a human-digestable representation of the PathType.  To make it human readable, we
     * need to convert the integers that show up in the path type into their string
     * representations, using the provided dictionaries (currently that's only the edge dictionary,
     * but it could expand as need dictates).
     */
     ...
     * edges in this method.  This is in the inner loop of the PathFollower, and so it will get run
     * a _lot_.  Even worse, the processing is distributed across threads by vertex, and if you
     * loop over all of the edges, you make the amount of computation done by each thread more
     * uneven, because vertices that have lots of edges tend to get more walks at them.  So,
     * really, try hard to avoid a loop over all of the edges.
     */
