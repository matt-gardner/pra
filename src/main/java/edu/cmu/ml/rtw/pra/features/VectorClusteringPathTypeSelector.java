...
public class VectorClusteringPathTypeSelector implements PathTypeSelector {
    ...
    public VectorClusteringPathTypeSelector(VectorPathTypeFactory factory,
    ...
    protected Collection<List<Pair<PathType, Integer>>> groupBySignature(
        ...
        for (Map.Entry<PathType, Integer> entry : pathCounts.entrySet()) {
            ...
            Pair<PathType, Integer> pair =
                    new Pair<PathType, Integer>(entry.getKey(), entry.getValue());
    ...
    /**
     * Does agglomerative clustering on the vectors found in each path type, to reduce the number
     * of path types kept.
     *
     ...
     */
    @VisibleForTesting
    protected List<Pair<PathType, Integer>> clusterPathTypes(
            List<Pair<PathType, Integer>> pathTypes) {
    ...
    protected Pair<PathType, Integer> combinePathTypes(Pair<PathType, Integer> first,
        ...
        for (int i = 0; i < numEdgeTypes; i++) {
            ...
            if (firstVector == null) {
                // We don't need to combine the vectors, so we know from the signature that these
                // two path types have the same edge here.  So just add it and move on.
            ...
            // weighted by their counts.  Weighting by count is a poor man's version of actually
            // keeping track of how many vectors are represented by a current cluster mean, in
            // order to avoid a lot of extra bookkeeping.  And it even seems like a reasonable
            // thing to do apart from bookkeeping considerations.
            ...
            Vector combinedVector = firstVector.multiply(firstCount)
                    .add(secondVector.multiply(secondCount));
