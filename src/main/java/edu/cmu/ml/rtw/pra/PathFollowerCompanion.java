...
public class PathFollowerCompanion extends TwoKeyCompanion {
    ...
    private boolean acceptableRow(int sourceNode, int targetNode, Set<Integer> sourceTargets,
    ...
    public List<MatrixRow> getFeatureMatrix(Map<Integer, Set<Integer>> sourcesMap) {
        ...
        for (Integer firstKey : distributions.keySet()) {
            ...
            Set<Integer> sourceTargets = sourcesMap.get(firstKey);
            if (sourceTargets == null) {
                sourceTargets = new HashSet<Integer>();
            }
            for (Integer secondKey : map.keySet()) {
