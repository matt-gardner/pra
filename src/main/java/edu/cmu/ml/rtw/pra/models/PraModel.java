...
public class PraModel {
    ...
    public List<Double> learnFeatureWeights(FeatureMatrix featureMatrix,
        ...
        if (config.outputBase != null) {
            try {
                ...
                positiveMatrix.outputToFile(base + "positive_matrix.tsv");
                negativeMatrix.outputToFile(base + "negative_matrix.tsv");
                unseenMatrix.outputToFile(base + "unseen_matrix.tsv");
            } catch(IOException e) {
    ...
    public void addNegativeEvidence(int numPositiveExamples,
        ...
        sampleUnseenExamples(numPositiveExamples,
                             ...
                             config);
        */
        weightUnseenExamples(numPositiveFeatures,
                             negativeMatrix,
                             unseenMatrix,
                             data,
                             alphabet,
    ...
    public void outputWeights(String filename,
        ...
        FileWriter writer = new FileWriter(filename);
        for (int i=0; i<pathTypes.size(); i++) {
            writer.write(pathTypes.get(i).encodeAsString() + "\t" + weights.get(i) + "\n");
        }
        writer.close();
    }
