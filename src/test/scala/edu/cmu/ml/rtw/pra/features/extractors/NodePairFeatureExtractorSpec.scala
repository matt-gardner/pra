...
class NodePairFeatureExtractorSpec extends FlatSpecLike with Matchers {
  ...
  def getPaths(
    ...
    edges: Seq[Seq[Int]],
    reverses: Option[Seq[Option[Seq[Boolean]]]] = None
  ): Set[Path] = {
    nodePairs.zip(edges).zipWithIndex.flatMap(nodePairAndEdges => {
      val nodePairs = nodePairAndEdges._1._1
      val edges = nodePairAndEdges._1._2
      val i = nodePairAndEdges._2
      nodePairs.map(nodePair => {
        val startNode = nodePair._1
        val nodes = new Array[Int](edges.size)
        for (i <- 0 until (nodes.length - 1)) nodes(i) = 1
        nodes(nodes.length - 1) = nodePair._2
        val _reverses = reverses match {
          case None => new Array[Boolean](edges.size)
          case Some(a) => a(i) match {
            case None => new Array[Boolean](edges.size)
  ...
  "AnyRelAliasOnlyFeatureExtractor" should "replace only paths with @ALIAS@" in {
    val edges = Seq(Seq(1, 3), Seq(2), Seq(5, 1, 5), Seq(5, 1, 2, 5))
  ...
  "ConnectedByMediatorFeatureExtractor" should "not return anything for non-mediator connections" in {
    val edges = Seq(Seq(2, 3), Seq(1), Seq(1, 2, 3), Seq(6, 7, 1))
    val reverses = Some(Seq(None, None, None, Some(Seq(false, true, false))))
    val nodePairs = Seq(Set((1,2)))
    val extractor = new ConnectedByMediatorFeatureExtractor(JNothing)
    val features = extractor.extractFeatures(instance, getPaths(nodePairs, edges, reverses))
    features.size should be(0)
  }
  ...
  it should "return the provided feature name when there is a path involving two mediator relations" in {
    ...
    // in a real graph.
    extractor.extractFeatures(instance, Set(Path(1, Array(3, 2), Array(6, 7), Array(false, false)))) should be(Seq(feature))
    extractor.extractFeatures(instance, Set(Path(1, Array(3, 2), Array(6, 7), Array(true, false)))) should be(Seq(feature))
    extractor.extractFeatures(instance, Set(Path(1, Array(3, 2), Array(6, 7), Array(false, true)))) should be(Seq(feature))
    extractor.extractFeatures(instance, Set(Path(1, Array(3, 2), Array(6, 7), Array(true, true)))) should be(Seq(feature))
  }
